<template>
  <div>
    <!-- Header -->
    <div class="mb-6">
      <h2 class="text-3xl font-bold text-gray-800">Одлуке суда</h2>
      <p class="text-gray-600 mt-2">Преглед одлука суда</p>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Датум пријема одлуке УС</th>
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Датум пријема жалбе</th>
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Датум пријема тужбе од УС</th>
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Статус</th>
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Тип пресуде</th>
            </tr>
          </thead>
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr v-for="(item, index) in data" :key="index" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {{ formatDate(item.datum_prijema_odluke_us) }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {{ formatDate(item.datum_prijema_zalbe) }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {{ formatDate(item.datum_prijema_tuzbe_od_us) }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {{ item.status_zalbe || '-' }}
              </td>
              ...
              <div v-for="(filter, index) in advancedFilters" :key="index" class="flex items-end gap-3 p-4 bg-gray-50 rounded-lg">
                ...
                <div class="flex-1">
                  ...
                  >
                    ...
                    <option value="tip_presude">Тип пресуде</option>
                  </select>
                </div>
...
const { loadSifarnici, getListFieldType, getSifarnikOptions } = useSifarnikFilters(['institucija_podnosioca_zalbe', 'status_zalbe', 'tip_presude']);
...
const getFieldType = (field) => {
  const dateFields = ['datum_prijema_odluke_us', 'datum_prijema_zalbe', 'datum_prijema_tuzbe_od_us'];
...
const fetchData = async (page = 1) => {
  try {
    ...
    const response = await axios.get('/izvestaj-odluke-suda', { params });
...
const exportExcel = () => {
  ...
  window.location.href = `${baseURL}/izvestaj-odluke-suda/export-excel?${params.toString()}`;
...
const exportPdf = () => {
  ...
  const baseURL = window.axios.defaults.baseURL || '/api';
  window.location.href = `${baseURL}/izvestaj-odluke-suda/export-pdf?${params.toString()}`;
