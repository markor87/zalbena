<template>
  <div>
    <!-- Header -->
    <div class="mb-6">
      <h2 class="text-3xl font-bold text-gray-800">Експедоване тужбе</h2>
      <p class="text-gray-600 mt-2">Преглед свих експедованих тужби</p>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Датум експедиције одговора ЖК</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr v-for="(item, index) in data" :key="index" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {{ formatDate(item.datum_ekspedicije_odgovora_zk) }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {{ item.status_zalbe || '-' }}
              </td>
...
const fetchData = async (page = 1) => {
  try {
    ...
    const response = await axios.get('/izvestaj-ekspedovane-tuzbe', { params });
