<template>
  <div>
    <!-- Header -->
    <div class="mb-6">
      <h2 class="text-3xl font-bold text-gray-800">Евиденција жалби по датуму пријема</h2>
      <p class="text-gray-600 mt-2">Преглед свих жалби сортираних по датуму пријема</p>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Датум пријема</th>
            ...
            <div v-for="(filter, index) in advancedFilters" :key="index" class="flex gap-3 items-start">
              ...
              <div class="flex-1">
                ...
                >
                  ...
                  <option value="datum_prijema_zalbe">Датум пријема</option>
...
const fetchData = async (page = 1) => {
  try {
    ...
    const response = await axios.get('/izvestaj-po-datumu-prijema', { params });
...
const exportExcel = () => {
  ...
  window.location.href = `${baseURL}/izvestaj-po-datumu-prijema/export-excel?${params.toString()}`;
...
const exportPdf = () => {
  ...
  const baseURL = window.axios.defaults.baseURL || '/api';
  window.location.href = `${baseURL}/izvestaj-po-datumu-prijema/export-pdf?${params.toString()}`;
