<template>
  <div>
    <!-- Header -->
    <div class="mb-6">
      <h2 class="text-3xl font-bold text-gray-800">Тужбе примљене од Управног суда Србије</h2>
      <p class="text-gray-600 mt-2">Преглед свих тужби примљених од Управног суда Србије</p>
    </div>
      ...
      <div class="flex items-end gap-4 mb-4">
        <div class="w-80">
          ...
          <input
            v-model="searchQuery"
            type="text"
            placeholder="Име, презиме, пријемни број, тип пресуде..."
...
const { loadSifarnici, getListFieldType, getSifarnikOptions } = useSifarnikFilters(['institucija_podnosioca_zalbe', 'tip_presude']);
...
const getFieldType = (field) => {
  const dateFields = ['datum_prijema_zalbe', 'datum_prijema_tuzbe_od_us'];
...
const fetchData = async (page = 1) => {
  try {
    ...
    const response = await axios.get('/izvestaj-tuzbe-od-us', { params });
...
const exportExcel = () => {
  ...
  window.location.href = `${baseURL}/izvestaj-tuzbe-od-us/export-excel?${params.toString()}`;
...
const exportPdf = () => {
  ...
  const baseURL = window.axios.defaults.baseURL || '/api';
  window.location.href = `${baseURL}/izvestaj-tuzbe-od-us/export-pdf?${params.toString()}`;
