        ...
        return Excel::download(new class($data) implements FromCollection, WithHeadings {
            ...
            public function collection()
            {
                return $this->data->map(function($item) {
                    return [
                        ...
                        'Датум пријема тужбе од УС' => $item->datum_prijema_tuzbe_od_us ? date('d.m.Y', strtotime($item->datum_prijema_tuzbe_od_us)) : '-',
                        'Датум експедиције одговора ЖК' => $item->datum_ekspedicije_odgovora_zk ? date('d.m.Y', strtotime($item->datum_ekspedicije_odgovora_zk)) : '-',
                        'Датум пријема одлуке УС' => $item->datum_prijema_odluke_us ? date('d.m.Y', strtotime($item->datum_prijema_odluke_us)) : '-',
                        'Број решења ЖК по пресуди УС' => $item->broj_resenja_zk_po_presudi_us ?? '-',
                        'Датум решења ЖК по пресуди УС' => $item->datum_resenja_zk_po_presudi_us ? date('d.m.Y', strtotime($item->datum_resenja_zk_po_presudi_us)) : '-',
            ...
            public function headings(): array
            {
                return [
                    ...
                    'Датум пријема тужбе од УС',
                    'Датум експедиције одговора ЖК',
                    'Датум пријема одлуке УС',
                    'Број решења ЖК по пресуди УС',
                    'Датум решења ЖК по пресуди УС',
                    'Доставница',
                    'Тип пресуде УС',
                    'Напомена',
                ];
